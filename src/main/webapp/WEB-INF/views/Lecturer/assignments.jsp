                  ...
                  <tbody id="assList" class="divide-y divide-gray-200 bg-white">
                    ...
                    <tr>
                      <td class="w-full max-w-0 py-4 px-3 text-sm font-medium text-gray-900 sm:w-auto sm:max-w-none">
                        ...
                        <p class="text-xs font-normal italic text-gray-400 md:hidden">${assignment.getFormattedStarted_at()} ${assignment.getFormattedTimeStarted_at()} - ${assignment.getFormattedEnded_at()} ${assignment.getFormattedTimeEnded_at()}</p>
                      </td>
                      ...
                      <td class="px-3 py-4 text-sm text-gray-500 md:table-cell hidden">${assignment.getFormattedEnded_at()} ${assignment.getFormattedTimeEnded_at()}</td>
        ...
        <script>
          ...
          $(document).ready(function() {
            ...
            $('#newAssBtn').on('click', function() {
              $('#ass-modal-title').html('Add New Assignment');
              $('#saveAss').val('add');
              $('#assAttachs').hide();
              modal.showModal();
              hugerte.init({
            ...
            $('.editAss').on('click', function() {
              ...
              $("#ass_late").click(function () {
                if ($(this).is(":checked")) {
                  $(this).attr("value", "1"); // Checkbox checked, set value to 1
                } else {
                  $(this).after('<input type="hidden" name="ass_late" value="0">'); // Add hidden input for unchecked state
                  $(this).removeAttr("value"); // Remove value attribute to prevent empty submission
                }
              });
