...
public class GenericRepository{
        ...
        try {
            ...
            return this.db.update(query, new PreparedStatementSetter() {
                ...
                public void setValues(PreparedStatement ps) throws SQLException {
                    ...
                    for(int i=fields.length; i<fields.length + condval.length; i++) {
                        int y = i-fields.length;
                        if(condtype[y].equalsIgnoreCase("varchar")) {
                            ps.setString(i+1, condval[y]);
                        }else if(condtype[y].equalsIgnoreCase("decimal")) {
                            ps.setDouble(i+1, Double.parseDouble(condval[y]));
                        }else if(condtype[y].equalsIgnoreCase("int")) {
    ...
    public String buildDeleteQuery(String table, String cond) {
        try {
            if(cond == null) {
